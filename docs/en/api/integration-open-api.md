...
| Header | Description |
| --- | --- |
| `NexaCard-Api-Key` | API Key (generated by the upstream site user under "API Permissions") |
| `NexaCard-Timestamp` | Unix timestamp (seconds) |
| `NexaCard-Signature` | HMAC-SHA256 signature (lowercase hex) |
...
| `{TIMESTAMP}` | Must match the `NexaCard-Timestamp` header |
...
| Header | Description |
| --- | --- |
| `NexaCard-Api-Key` | API Key configured in Site A's "Site Connection" |
| `NexaCard-Timestamp` | Unix timestamp (seconds) |
