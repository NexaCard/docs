...
The `nexacard-api` binary ships with a built-in `admin` subcommand for server-side operations: list administrators, reset 2FA, reset passwords. The most common use case is **emergency recovery when a super admin forgets their password or loses their 2FA device**.
...
> The subcommand shares the same configuration as `nexacard-api` (server startup). It does not start HTTP / worker / embedded frontend, does not print the banner, and exits immediately after the subcommand finishes. Only the database is initialized.
...
```bash
nexacard-api admin list-admins                            # List all admins
nexacard-api admin reset-2fa --username <name>            # Reset 2FA for a specific admin
nexacard-api admin reset-password --username <name>       # Interactive password reset (two-time confirmation)
nexacard-api admin reset-password --username <name> --password <new>  # Provide new password inline
```
...
```bash
nexacard-api admin list-admins
```
...
```bash
nexacard-api admin reset-2fa --username admin
```
...
```bash
nexacard-api admin reset-password --username admin
新密码:
再次输入:
...
```bash
nexacard-api admin reset-password --username admin --password 'StrongPwd!2026'
```
...
```bash
echo 'StrongPwd!2026' | nexacard-api admin reset-password --username admin
```
...
```bash
cd /opt/nexacard-api          # Same working directory as the server (so config.yml is readable)
./nexacard-api admin list-admins
./nexacard-api admin reset-password --username admin
```
...
```bash
# Assuming WorkingDirectory=/opt/nexacard-api, User=nexacard
sudo -u nexacard /opt/nexacard-api/nexacard-api admin reset-password --username admin
```
...
docker compose exec api ./nexacard-api admin list-admins
docker compose exec api ./nexacard-api admin reset-password --username admin

# Or a one-shot container
docker compose run --rm api ./nexacard-api admin reset-2fa --username admin
```
...
```bash
sudo -u www /www/wwwroot/nexacard-api/nexacard-api admin reset-password --username admin
```
...
- **Shell access ≈ super admin access**: anyone who can run `nexacard-api admin ...` can change the super admin password or clear 2FA. Tightly control server shell access.
